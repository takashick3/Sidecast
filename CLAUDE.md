...
- **次にやること**: `Sidecast/` に Xcode プロジェクト（SwiftUI `MenuBarExtra`、`LSUIElement`）を作成し、PoC の `PoCCommon` / `rebuild` のロジックを `AudioEngine` / `ProcessMonitor` / `DeviceMonitor` に移植。UI は ON/OFF・対象アプリ選択・gain スライダー
